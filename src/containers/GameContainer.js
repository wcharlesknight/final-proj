...
class Game extends Component {
    ...
    if (e.keyCode === 49) {
      if (this.state.bonusPoints - 2 >= 0) {
        this.setState((prevState) => {
          return { bonusPoints: prevState.bonusPoints - 2, bonusOn: 2 };
        });
      } else this.errorMessages("Not enough bonus points");
    }
    if (e.keyCode === 50) {
      if (this.state.bonusPoints - 6 >= 0) {
        this.setState((prevState) => {
          return { bonusPoints: prevState.bonusPoints - 6, bonusOn: 3 };
        });
      } else this.errorMessages("Not enough bonus points");
    }
    if (e.keyCode === 51) {
  ...
  timer = () => {
    // setInterval(() => {
    if (this.props.round < 7) {
      if (this.props.timer === 0) {
        this.getWord();
        this.props.multiGame === true
          ? this.props.multiRound(1)
          : this.props.nextRound();
        this.props.multiGame === true
          ? this.props.multiTimer(7)
          : this.props.changeTimer(7);
        this.props.multiGame === true
          ? this.props.multiReset()
          : this.props.resetWords();
        this.setBonus();
      } else {
        this.props.multiGame === true
          ? this.props.multiTimer(-1)
  ...
  postGame = () => {
    ...
    fetch("http://localhost:3000/games", configW)
      .then((resp) => resp.json())
      .then((game) =>
        this.props.addGame({ user_id: game.user_id, score: game.score })
      );
  };
  ...
  checkWord = (e) => {
    ...
    if (e.target.children[0].value.length >= 3) {
      fetch(
        ` https://wordsapiv1.p.rapidapi.com/words/${e.target.children[0].value}`,
        {
          headers: {
            "x-rapidapi-key":
              "a87f5b1016mshe5a42db9c818a58p1f3c56jsn8b59ca641ffc",
        ...
        if (uniqW.sort().join("") == uniq.sort().join("")) {
          ...
          if (validity == 0) {
            if (this.props.gameWord.length == 2) {
              this.setState((prevState) => {
                return { bonusPoints: prevState.bonusPoints + 1, bonusWord: 2 };
              });
            }
            if (this.props.gameWord.length == 5) {
              this.setState((prevState) => {
                return { bonusPoints: prevState.bonusPoints + 2, bonusWord: 3 };
              });
            }
            if (this.props.gameWord.length >= 8) {
              this.setState((prevState) => {
                return { bonusPoints: prevState.bonusPoints + 3, bonusWord: 4 };
              });
            }
            this.wordState(word);
            word.split("").forEach((letter) => {
              this.fetchLetter(letter);
            });
          } else return this.errorMessages("All letters do not match");
        } else return this.errorMessages("All letters do not match");
      } else return this.errorMessages("You have alredy used that word");
    } else {
  ...
  addScore = (points) => {
    if (this.state.bonusWord === 2 && points[0].point > 0) {
      this.errorMessages(`You got 1 bonus point!`);
    }
    if (this.state.bonusWord === 3 && points[0].point > 0) {
      this.errorMessages(`You got 2 bonus points!`);
    }
    if (this.state.bonusWord === 4 && points[0].point > 0) {
      this.errorMessages(`You got 3 bonus points!`);
    }
    ...
    points[0].point > 0
      ? this.props.addPoint(
          points[0].point * this.state.bonusWord * this.state.bonusOn
        )
      : this.props.addPoint(points[0].point);
    if (this.props.multiGame === true) {
      this.props.addMultiPoints(
        points[0].point * this.state.bonusWord * this.state.bonusOn
      );
    }
    this.resetScored();
        ...
        {this.props.gameOver === false ? (
          <Container id="canvas" fluid className="full-height app-font">
            <Row className="box-fixed text-center black-back purple-border white-text">
              {this.props.multiGame === true ? (
                <div>
                  <h1 className="opponent-score">
                    P1: {this.props.playerOneScore}
                  </h1>
                  <h1 className="opponent-score-2">
                    P2: {this.props.playerTwoScore}
                  </h1>
                </div>
              ) : //SCORE
              this.state.scored === false ? (
                <div className="position-fixed score-top">
                  Score: {this.props.score}
                </div>
              ) : (
                <div className="position-fixed score-top-scored">
                  Score: {this.props.score}
                </div>
              )}
              {this.props.round === 0 ? (
                <div className="position-fixed countdown">
                  Game starts in: {this.props.timer}
                </div>
              ) : (
                <div className="position-fixed countdown invisible">
                  Game starts in: {this.props.timer}
                </div>
              )}
              {/* FLYING POINTS */}
              <div className="flying-words">
                {this.props.points.map((point) =>
                  point > 0 ? (
                    <Button className="m-1 green magictime tinUpOut z-pos border border-white border-5">
                      + {point}
                    </Button>
                  ) : (
                    <Button className="red magictime tinDownOut z-pos border border-white border-5">
                      {point}
                    </Button>
                  )
                )}
              ...
              {/* CURRENT WORD BOX   */}
              {this.props.multiGame === true ? (
                <div className="words m-2">
                  {this.props.curMultiWord.map((word, index) => (
                    <a key={index} className="magictime spaceInUp tile m-1">
                      {word.character}
                ...
                <div className="words m-2">
                  {this.state.currentWord.map((word, index) => (
                    <a key={index} className="magictime spaceInUp tile m-1">
                      {word.character}
                      <span>{word.point}</span>
                    </a>
              ...
              )}
              {
                ...
                this.state.bonusUsed === true ? (
                  ...
                  <h3
                    className={
                      this.state.bonusOn * this.state.bonusWord > 1
                        ? "multiplier magictime vanishIn"
                        : "multiplier invisible"
                    }
                  >
                    Current Multiplier:
                    {`${this.state.bonusOn * this.state.bonusWord}x`}
                  </h3>
                )
              }
            </Row>
            <Row className="box-fixed-2 back-image">
              <Col className="text-center ">
                <Stats
                  bonus={this.state.bonusOn * this.state.bonusWord}
                  bonusPoints={this.state.bonusPoints}
                />
                <Button className="button" onClick={() => this.exitGame()}>
                  Exit Game
                </Button>
              </Col>
              <Col className="text-center ">
                ...
                {this.state.error === "" ? (
                  <div className="white-error-2 invisible">
                    {this.state.error} hi
                  </div>
                ) : (
                  <div className="white-error-2">{this.state.error}</div> 
                )}
              </Col>
              <Col className="text-center ">
                <Card className="used-words ">
                  <Card.Header className="back-image-real white-text purple-border">
                    {/* <RiFileWordFill className="lit-word-left" size="17" />  */}
                    Used Words
                  </Card.Header>
                  {/* <RiFileWordFill className="bottom-r-1" /> */}
                  <Card.Body className="card-back">
                    {this.props.gameWord.map((word) => (
                      <Card.Text className=" m-0">{word}</Card.Text>
                    ))}
                  </Card.Body>
            ...
            <Row className="box-fixed-3 black-back purple-border-top">
              <Card className="text-center shadow">
                <Card.Header className="purple-border">Power-ups</Card.Header>

                <Card.Body>
                  <h5>
                    <FcFlashAuto /> Press '1' to use 2x multiplier (2 bonus
                    points)
                  </h5>
                  <h5>
                    <GiPowerLightning /> Press '2' to use 3x multiplier (6 bonus
                    points)
                  </h5>
                </Card.Body>
              </Card>
          ...
          <GameOver
            winner={
              this.props.playerOneScore > this.props.playerTwoScore
                ? "P1"
                : "P2"
            }
            gameStart={this.toggleGame}
          />
        )}
      </div>
    );
