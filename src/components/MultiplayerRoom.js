...
class PreMultiplayerRoom extends Component {
  ...
  setPlayer = () => {
    console.log(
      this.props.playerOne.user_id != localStorage.id,
      "comparison on setplayer"
    );
  ...
  setupSubscription = () => {
    this.setState({
      gameChannel: consumer.subscriptions.create(
        { channel: "MultiGamesChannel", id: this.state.streamId },
        {
          received(data) {
    ...
    if (playerOne)
      if (playerOne.user_id.toString() === localStorage.id) {
        ...
        fetch(`${API_ROOT}/players/${playerOne.id}`, {
          ...
          .then(
            fetch(`${API_ROOT}/multi_games/${this.state.streamId}`, {
              method: "DELETE",
              headers: HEADERS,
            })
          );
  ...
  addMultiPoints = (points) => {
    this.state.gameChannel.send({
      multi_score: { name: this.props.player, points: points },
    });
  };
    ...
    return (
      <div>
        {this.props.multiGame === true ? (
          <div>
            <GameContainer
              multiReset={this.multiReset}
              multiRound={this.multiRound}
              endGame={this.endGame}
              multiTimer={this.multiTimer}
              curWord={this.curWord}
              addMultiPoints={this.addMultiPoints}
            />
          </div>
        ) : (
